...
最新改版增加统一上传工作区、三栏固定导航、聊天标签与完整生命周期；见 [ADR 0005](adr/0005-unified-workspace-and-chat-lifecycle.md)。全自动、模型实调与真人评分继续分别登记，本轮不宣称比赛完整验收或一等奖。

本轮状态以 `current_run.json` 与 [工作台升级记录](workspace_upgrade_20260923.md) 为准。下方矩阵保留历史开发记录；历史“已验证”只适用于其对应代码和数据。最新轮新增独立助手、统一界面与真实状态栏，并修复季度归因、对标总额口径、知识范围隔离及模型连接覆盖问题。

| 项目 | 当前证据与边界 |
|---|---|
| 成本计算、季度归因、总额对标 | 本轮离线回归覆盖；原数据不被修改 |
| 助手与配置 | 独立路由、结构化输出、事实绑定、会话持久化、操作确认；真实本地 HTTP 模拟提供方联调，不能称云模型实调 |
| 网页与交互 | 实际浏览器多视口检查，构建通过；仍需团队真人审美评审 |
| 知识导入 | 保留原件、来源位置、按范围索引、失败不切换；扫描页缺文字明确报需 OCR |
| 行业扩展 | 无示例数据的通用制造行业包；演示只使用赛题数据，其他包仍为测试夹具 |
| 正式部署 | Compose 与启动器修复；当前环境没有 Docker 引擎，净机部署待验证 |
| 比赛三场景、加分质量 | 本轮不宣称整体验收；S1/S2/S3 的新云模型生成、真人评分、预测及多模型增益均单独待评 |
