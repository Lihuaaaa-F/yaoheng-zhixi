...
export default function FilePreview({ importId, onClose }: { importId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    if(data?.format !== 'pdf' || !data?.url?.startsWith('/api/')) return;
    const controller = new AbortController(); let url = '';
    fetch(data.url, {headers:apiHeaders(),signal:controller.signal}).then(r=>{if(!r.ok)throw new Error(`文件读取失败（HTTP ${r.status}）`);return r.blob()}).then(blob=>{if(!controller.signal.aborted){url=URL.createObjectURL(blob);setPdfUrl(url)}}).catch(e=>{if(!controller.signal.aborted)setError(e.message)});
    return()=>{controller.abort();if(url)URL.revokeObjectURL(url);setPdfUrl('')};
  }, [data?.url]);
  ...
  return <Modal title="原始文件预览" open onCancel={onClose} footer={null} width={1000} style={{top:24}} styles={{body:{maxHeight:'80vh',overflow:'auto'}}} destroyOnHidden>
      ...
      {data?.format === 'pdf' && pdfUrl && <iframe className="preview-pdf" title="PDF 原始文件" src={pdfUrl} />}
