...
export function MetricDetails({value}:{value:any}) {return <><p className="muted">{value.product} {value.factory} {value.period?.start}{value.period?.end&&value.period?.end!==value.period?.start?` 至 ${value.period.end}`:""}</p>{value.row_keys?.length>0&&<p>数据来源：{[...new Set(value.row_keys.map((key:string)=>key.replace(/:\d+$/,'')))].join("；")} · {value.product} · {value.period?.end??"所选期间"}</p>}<dl className="task-details">{value.value!==undefined&&<><dt>指标数值</dt><dd>{fmt(value.value)} {value.unit}</dd></>}{value.formula&&<><dt>计算口径</dt><dd>{cleanText(value.formula)}</dd></>}{value.comparison_period&&<><dt>比较期间</dt><dd>{Array.isArray(value.comparison_period)?value.comparison_period.join(' 至 '):String(value.comparison_period)}</dd></>}{value.numerator!==undefined&&<><dt>分子</dt><dd>{fmt(value.numerator)}</dd></>}{value.denominator!==undefined&&<><dt>分母</dt><dd>{fmt(value.denominator)}</dd></>}</dl></>}
