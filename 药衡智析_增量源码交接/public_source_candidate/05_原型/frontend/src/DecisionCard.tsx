...
export default function DecisionCard({ selection, onApplied, onError }: {
    ...
    useEffect(() => {
        ...
        setData(null); setApplied(null);setExplaining(false);
    ...
    return <section className="panel decision-card" aria-label="Agent 分析决策">
        ...
        <p className="muted">决策由确定性策略 {data.policy_version} 作出{data.advisory_status === 'PASS' ? `，说明依据由 ${data.advisory_model} 选择，文字由规则生成` : '，未使用模型说明（结论不受影响）'}。决策仅是系统建议；正式发送与人工审核流程不变。</p>
